...
if __name__ == '__main__':
    ...
    dtrain = xgb.DMatrix(X_train, label=y_train, sensitive_feature=s_train)
    dtest = xgb.DMatrix(X_test, label=y_test, sensitive_feature=s_test)
    ...
    for fair_reg in tqdm(np.linspace(0.0, 0.8, num=100)):
        ...
        fair_result = xgb.train(fair_params, dtrain, num_round, obj=fairbceobj(fair_reg=fair_reg))
        fair_results[fair_reg] = fair_result

    results = {}
    print('Predictions with fair models')
    for fair_reg in tqdm(np.linspace(0.0, 0.8, num=100)):
        y = fair_results[fair_reg].predict(dtest)
        eval_df = pd.DataFrame([y_test, y, s_test]).transpose()
