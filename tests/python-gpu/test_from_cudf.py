...
def dmatrix_from_cudf(
    input_type, DMatrixT, missing=np.NAN, data_split_mode=DataSplitMode.ROW
):
    ...
    dtrain = DMatrixT(
        cd, missing=missing, label=cd_label, data_split_mode=data_split_mode
    )
    if data_split_mode == DataSplitMode.ROW:
        assert dtrain.num_col() == kCols
    else:
        assert dtrain.num_col() == kCols * xgb.collective.get_world_size()
    assert dtrain.num_row() == kRows


def _test_from_cudf(DMatrixT, data_split_mode=DataSplitMode.ROW):
    """Test constructing DMatrix from cudf"""
    import cudf

    world_size = xgb.collective.get_world_size()

    dmatrix_from_cudf(np.float32, DMatrixT, np.NAN, data_split_mode)
    dmatrix_from_cudf(np.float64, DMatrixT, np.NAN, data_split_mode)

    dmatrix_from_cudf(np.int8, DMatrixT, 2, data_split_mode)
    dmatrix_from_cudf(np.int32, DMatrixT, -2, data_split_mode)
    dmatrix_from_cudf(np.int64, DMatrixT, -3, data_split_mode)

    cd = cudf.DataFrame({"x": [1, 2, 3], "y": [0.1, 0.2, 0.3]})
    dtrain = DMatrixT(cd, data_split_mode=data_split_mode)

    if data_split_mode == DataSplitMode.ROW:
        assert dtrain.feature_names == ["x", "y"]
        assert dtrain.feature_types == ["int", "float"]
    else:
        assert dtrain.feature_names == tm.column_split_feature_names(
            ["x", "y"], world_size
        )
        assert dtrain.feature_types == ["int", "float"] * world_size

    series = cudf.DataFrame({"x": [1, 2, 3]}).iloc[:, 0]
    assert isinstance(series, cudf.Series)
    dtrain = DMatrixT(series, data_split_mode=data_split_mode)

    if data_split_mode == DataSplitMode.ROW:
        assert dtrain.feature_names == ["x"]
        assert dtrain.feature_types == ["int"]
    else:
        assert dtrain.feature_names == tm.column_split_feature_names(["x"], world_size)
        assert dtrain.feature_types == ["int"] * world_size

    with pytest.raises(ValueError, match=r".*multi.*"):
        dtrain = DMatrixT(cd, label=cd, data_split_mode=data_split_mode)
    ...
    dtrain = DMatrixT(X, data_split_mode=data_split_mode)
    if data_split_mode == DataSplitMode.ROW:
        assert dtrain.num_col() == 1
    else:
        assert dtrain.num_col() == 1 * world_size
    assert dtrain.num_row() == 5

    # Boolean is not supported.
    X_boolean = cudf.DataFrame({"x": cudf.Series([True, False])})
    with pytest.raises(Exception):
        dtrain = DMatrixT(X_boolean, data_split_mode=data_split_mode)

    y_boolean = cudf.DataFrame({"x": cudf.Series([True, False, True, True, True])})
    with pytest.raises(Exception):
        dtrain = DMatrixT(X_boolean, label=y_boolean, data_split_mode=data_split_mode)
...
class TestFromColumnar:
    ...
    def test_simple_dmatrix_from_cudf_column_split(self):
        tm.run_with_rabit(
            world_size=3,
            test_fn=_test_from_cudf,
            DMatrixT=xgb.DMatrix,
