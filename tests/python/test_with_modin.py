...
class TestModin(unittest.TestCase):
    ...
    def test_cv_as_modin(self):
        dm = xgb.DMatrix(dpath + 'agaricus.txt.train')
        params = {'max_depth': 2, 'eta': 1, 'verbosity': 0,
                  'objective': 'binary:logistic'}

        cv = xgb.cv(params, dm, num_boost_round=10, nfold=10, as_pandas=False, as_modin=True)
        assert isinstance(cv, md.DataFrame)
        exp = md.Index([u'test-error-mean', u'test-error-std',
        ...
        cv = xgb.cv(params, dm, num_boost_round=10, nfold=10,
                    verbose_eval=True, as_pandas=False, as_modin=True)
        assert isinstance(cv, md.DataFrame)
        exp = md.Index([u'test-error-mean', u'test-error-std',
                        u'train-error-mean', u'train-error-std'])
        assert len(cv.columns.intersection(exp)) == 4
        cv = xgb.cv(params, dm, num_boost_round=10, nfold=10,
                    verbose_eval=True, show_stdv=False, as_pandas=False, as_modin=True)
        assert isinstance(cv, md.DataFrame)
        exp = md.Index([u'test-error-mean', u'test-error-std',
                        u'train-error-mean', u'train-error-std'])
        assert len(cv.columns.intersection(exp)) == 4
        ...
        params = {'max_depth': 2, 'eta': 1, 'verbosity': 0,
                  'objective': 'binary:logistic', 'eval_metric': ['auc']}
        cv = xgb.cv(params, dm, num_boost_round=10, nfold=10,
                    as_pandas=False, as_modin=True, early_stopping_rounds=1)
        assert 'eval_metric' in params
        assert 'auc' in cv.columns[0]
        ...
        params = {'max_depth': 2, 'eta': 1, 'verbosity': 0,
                  'objective': 'binary:logistic'}
        cv = xgb.cv(params, dm, num_boost_round=10, nfold=10,
                    as_pandas=False, as_modin=True, metrics='auc')
        assert 'auc' in cv.columns[0]
        ...
        params = {'max_depth': 2, 'eta': 1, 'verbosity': 0,
                  'objective': 'binary:logistic', 'eval_metric': ['auc']}
        cv = xgb.cv(params, dm, num_boost_round=10, nfold=10,
                    as_pandas=False, as_modin=True, metrics='error')
        assert 'eval_metric' in params
