...
class TestPandasColumnSplit:
    ...
    def verify_pandas():
        ...
        assert dm.feature_names == [
            f"{rank}.{feature}" for rank in range(world_size) for feature in features
        ]
        ...
        expected = [
            f"{rank}.{feature}" for rank in range(world_size) for feature in ["1", "2"]
        ]
