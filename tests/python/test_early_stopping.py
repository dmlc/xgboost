...
class TestEarlyStopping(unittest.TestCase):
    ...
    def test_early_stopping_nonparallel2(self):
        ...
        clf1 = xgb.XGBClassifier()
        clf1.fit(X_train, y_train, early_stopping_rounds=5, early_stopping_threshold=1E-2, early_stopping_limit=1.0, eval_metric="auc",
                 eval_set=[(X_test, y_test)])
        clf2 = xgb.XGBClassifier()
        clf2.fit(X_train, y_train, early_stopping_rounds=4, early_stopping_threshold=1E-2, early_stopping_limit=1.0, eval_metric="auc",
                 eval_set=[(X_test, y_test)])
        # should be the same
        assert clf1.best_score == clf2.best_score
        assert clf1.best_score != 1
        # check overfit
        clf3 = xgb.XGBClassifier()
        clf3.fit(X_train, y_train, early_stopping_rounds=10, early_stopping_threshold=1E-2, early_stopping_limit=1.0, eval_metric="auc",
                 eval_set=[(X_test, y_test)])
