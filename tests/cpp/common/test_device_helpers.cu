...
void TestSegmentedArgSort() {
  ...
  dh::LaunchN(dh::CurrentDevice(), sorted_idx.size(),
  ...
  for (size_t i = 1; i < kGroups + 1; ++i) {
    auto group_idx = Span<size_t>(h_sorted_index).subspan(offset_ptr[i-1], offset_ptr[i] - offset_ptr[i-1]);
