...
class DeviceSketchWithHessianTest
  ...
  void Check(Context const* ctx, std::shared_ptr<DMatrix> p_fmat, bst_bin_t n_bins,
 ...
 protected:
  ...
  void TestLTR(Context const* ctx, bst_row_t n_samples, bst_bin_t n_bins) const {
  ...
  void TestRegression(Context const* ctx, bst_row_t n_samples, bst_bin_t n_bins) const {
    auto x = GenerateRandom(n_samples, n_features_);
    auto p_fmat = GetDMatrixFromData(x, n_samples, n_features_);
    ...
    this->Check(ctx, p_fmat, n_bins, hessian, w);
  }
};
