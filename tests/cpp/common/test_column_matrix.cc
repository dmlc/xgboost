  ...
  for (int32_t max_num_bin : max_num_bins) {
    ...
    for (auto i = 0ull; i < dmat->Info().num_row_; i++) {
      for (auto j = 0ull; j < dmat->Info().num_col_; j++) {
        auto col = column_matrix.GetColumn<false>(j);
...
TEST(SparseColumn, Test) {
  ...
  for (int32_t max_num_bin : max_num_bins) {
    ...
    auto col = column_matrix.GetColumn<true>(0);
    CheckSparseColumn(*col.get(), gmat);
  }
}

inline void CheckColumWithMissingValue(const Column& col_input, const GHistIndexMatrix& gmat) {
  const DenseColumn<true>& col = static_cast<const DenseColumn<true>&>(col_input);
  for (auto i = 0ull; i < col.Size(); i++) {
