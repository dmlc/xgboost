...
void VerifySampling(size_t page_size,
  ...
  if (fixed_size_sampling) {
    ...
    EXPECT_NEAR(sample.sample_rows, sample_rows, kRows * 0.05f);
    EXPECT_NEAR(sample.page->n_rows, sample_rows, kRows * 0.05f);
    EXPECT_NEAR(sample.gpair.size(), sample_rows, kRows * 0.05f);
  }
