...
TEST(Objective, GPUSoftmaxMultiClassGPair) {
  xgboost::ObjFunction * obj = xgboost::ObjFunction::Create("gpu:multi:softmax");
  ...
  CheckObjFunction(obj,
		   ...
		   {1.0, 0.0},		// labels
		   {1.0, 1.0},		// weights
		   {0.24, -0.91, 0.66, -0.33f, 0.09f, 0.24f}, // grad
		   {0.36, 0.16, 0.44, 0.45, 0.16, 0.37});     // hess

  ASSERT_NO_THROW(obj->DefaultEvalMetric());
}

TEST(Objective, GPUSoftmaxMultiClassBasic) {
  xgboost::ObjFunction * obj = xgboost::ObjFunction::Create("gpu:multi:softmax");
...
TEST(Objective, GPUSoftprobMultiClassBasic) {
  xgboost::ObjFunction * obj = xgboost::ObjFunction::Create("gpu:multi:softprob");
  std::vector<std::pair<std::string, std::string>> args
    {std::pair<std::string, std::string>("num_class", "3")};
  obj->Configure(args);
