...
TEST(c_api, CSRAdapter) {
  ...
  data::SimpleDMatrix dmat(&adapter,-1);
...
TEST(c_api, DenseAdapterPerformance) {
  int m = 100000;
  int n = 1000;
  ...
  common::Timer t;
  XGDMatrixCreateFromMatAdapter(
      data.data(), m, n, std::numeric_limits<float>::quiet_NaN(), &handle,1);
  t.Stop();
  delete static_cast<std::shared_ptr<xgboost::DMatrix> *>(handle);
  t.PrintElapsed("Adapter - 1 thread");
  t.Reset();
  XGDMatrixCreateFromMatAdapter(
      data.data(), m, n, std::numeric_limits<float>::quiet_NaN(), &handle,-1);
  t.Stop();
  delete static_cast<std::shared_ptr<xgboost::DMatrix> *>(handle);
  t.PrintElapsed("Adapter - all threads");
  t.Reset();
  ...
  XGDMatrixCreateFromMat(
      data.data(), m, n, std::numeric_limits<float>::quiet_NaN(), &handle);
  t.Stop();
  delete static_cast<std::shared_ptr<xgboost::DMatrix> *>(handle);
  t.PrintElapsed("Default");
}
