...
static inline void CheckLossOverGridPoints(
                      ...
                      const std::map<AFTDistributionType, std::vector<double>>& reference_values) {
  const int num_point = 20;
  const double log_y_low = 1.0;
  const double log_y_high = 15.0;
  for (auto dist_type : { AFTDistributionType::kNormal, AFTDistributionType::kLogistic,
    ...
    for (int i = 0; i < num_point; ++i) {
      ...
      ASSERT_NEAR(loss_val, reference_values.at(dist_type)[i], 1e-4);
    }
  }
}
...
TEST(AFTLoss, Uncensored) {
  ...
  // Reference values for AFT loss
  const std::map<AFTDistributionType, std::vector<double>> reference_values{
    {AFTDistributionType::kNormal,
    ...
    {AFTDistributionType::kExtreme,
      { 50.6931, 31.2065, 19.7177, 13.0281, 9.2183, 7.1365, 6.0916, 5.6688, 5.6195, 5.7941,
        6.1031, 6.4929, 6.9310, 7.3981, 7.8827, 8.3778, 8.8791, 9.3842, 9.8916, 10.4003 } }
  };
  CheckLossOverGridPoints(true_label_lower_bound, true_label_upper_bound, reference_values);
}
  ...
  const std::map<AFTDistributionType, std::vector<double>> reference_values{
    {AFTDistributionType::kNormal,
      {0.0107, 0.0373, 0.1054, 0.2492, 0.5068, 0.9141, 1.5003, 2.2869, 3.2897, 4.5196, 5.9846,
       7.6902, 9.6405, 11.8385, 14.2867, 16.9867, 19.9399, 23.1475, 26.6103, 30.3290} },
    {AFTDistributionType::kLogistic,
      {0.0953, 0.1541, 0.2451, 0.3804, 0.5717, 0.8266, 1.1449, 1.5195, 1.9387, 2.3902, 2.8636,
       3.3512, 3.8479, 4.3500, 4.8556, 5.3632, 5.8721, 6.3817, 6.8918, 7.402} },
    {AFTDistributionType::kExtreme,
...
TEST(AFTLoss, RightCensored) {
  ...
  const std::map<AFTDistributionType, std::vector<double>> reference_values{
    ...
    {AFTDistributionType::kExtreme,
      {29.9998, 18.0015, 10.8018, 6.4817, 3.8893, 2.3338, 1.4004, 0.8403, 0.5042, 0.3026, 0.1816,
