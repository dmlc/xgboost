 ...
 public:
  void Run(float hw_comp_ratio) {
    ...
    for (std::size_t i = 0, n = full_pages.size(); i < n; ++i) {
      ...
      auto acc_f = impl_f->GetHostEllpack(&ctx, &buf_f);

      std::vector<common::CompressedByteT> buf_c;
      auto acc_c = impl_c->GetHostEllpack(&ctx, &buf_c);
...
TEST_P(EllpackDecompTest, Basic) {
  auto ctx = MakeCUDACtx(0);
  auto hw_comp_ratio = this->GetParam();
  this->Run(hw_comp_ratio);
}
