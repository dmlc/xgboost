...
TEST(SparsePageDMatrix, ColAccess) {
  ...
  for (auto col_batch : dmat->GetBatches(xgboost::kSortedCSC).Of<xgboost::SparsePage>()) {
  ...
  // Loop over the batches and assert the data is as expected
  for (auto col_batch : dmat->GetBatches(xgboost::kCSC).Of<xgboost::SparsePage>()) {
    EXPECT_EQ(col_batch.Size(), dmat->Info().num_col_);
    EXPECT_EQ(col_batch[1][0].fvalue, 10.0f);
    EXPECT_EQ(col_batch[1].size(), 1);
  }
...
TEST(SparsePageDMatrix, ColAccessBatches) {
  ...
  for (auto const& page : dmat->GetBatches(xgboost::kCSC).Of<xgboost::SparsePage>()) {
