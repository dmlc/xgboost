...
TEST(Predictor, CpuTreeshapUsesQuadratureShapCPU) {
  Context ctx;
  size_t constexpr kRows = 256;
  size_t constexpr kCols = 1;
  ...
  HostDeviceVector<float> extracted_shap;
  learner->Predict(dmat, false, &extracted_shap, 0, 0, false, false, true, false, false);

  LearnerModelParam mparam;
  auto gbtree = LoadGBTreeModel(learner.get(), dmat->Ctx(), args, &mparam);

  HostDeviceVector<float> quadrature_shap;
  interpretability::cpu_impl::QuadratureShapValues(dmat->Ctx(), dmat.get(), &quadrature_shap,
                                                   *gbtree, 0, nullptr, 8);

  auto const& h_extracted = extracted_shap.ConstHostVector();
  auto const& h_quadrature = quadrature_shap.ConstHostVector();
  ASSERT_EQ(h_extracted.size(), h_quadrature.size());
  for (size_t i = 0; i < h_extracted.size(); ++i) {
    EXPECT_NEAR(h_extracted[i], h_quadrature[i], 1e-6f);
  }

  CheckShapAdditivity(kRows, kCols, extracted_shap, margin_predt);
}

TEST(Predictor, CpuTreeshapInteractionsUseQuadratureShapCPU) {
  Context ctx;
  ...
  HostDeviceVector<float> extracted_interactions;
  learner->Predict(dmat, false, &extracted_interactions, 0, 0, false, false, false, false, true);

  LearnerModelParam mparam;
  auto gbtree = LoadGBTreeModel(learner.get(), dmat->Ctx(), args, &mparam);

  HostDeviceVector<float> quadrature_interactions;
  interpretability::cpu_impl::QuadratureShapInteractionValues(
      dmat->Ctx(), dmat.get(), &quadrature_interactions, *gbtree, 0, nullptr, 8);

  auto const& h_extracted = extracted_interactions.ConstHostVector();
  auto const& h_quadrature = quadrature_interactions.ConstHostVector();
  ASSERT_EQ(h_extracted.size(), h_quadrature.size());
  for (size_t i = 0; i < h_extracted.size(); ++i) {
    EXPECT_NEAR(h_extracted[i], h_quadrature[i], 1e-6f);
  }

  CheckShapAdditivity(kRows, kCols, extracted_interactions, margin_predt);
}
