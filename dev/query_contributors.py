...
if len(sys.argv) != 5:
    print(
        f"Usage: {sys.argv[0]} [starting commit/tag] [ending commit/tag] [GitHub username] "
        + "[GitHub password]"
    )
    sys.exit(1)
...
for line in git.log(
    f"{from_commit}..{to_commit}", "--pretty=format:%s", "--reverse", "--first-parent"
):
    ...
    if m:
        ...
        def process_commit_list(commit_list):
            try:
                ...
                prompt = (
                    f"Error fetching contributors for PR #{pr_id}. Enter it manually, "
        ...
        paginate_request(
            f"https://api.github.com/repos/dmlc/xgboost/pulls/{pr_id}/reviews",
            process_review_list,
        )
        paginate_request(
            f"https://api.github.com/repos/dmlc/xgboost/issues/{pr_id}/comments",
