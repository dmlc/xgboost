...
def make_pyproject(*, use_suffix: str, require_nccl_dep: str, create_stub: bool = False) -> None:
    if use_suffix == "cpu" and require_nccl_dep != "na":
        raise ValueError("xgboost-cpu cannot require NCCL dependency. When setting --use-suffix='cpu', you must also set --require-nccl-dep='na'.")
    if use_suffix in CUDA_VARIANTS and require_nccl_dep in CUDA_VARIANTS and use_suffix != require_nccl_dep:
        raise ValueError(
    ...
    if create_stub:
        ...
        if require_nccl_dep != "na":
            raise ValueError("To create a stub package, --require-nccl-dep must be 'na'")
    ...
    pyproject = pyproject.replace(NAME, f"xgboost-{use_suffix}" if use_suffix != "na" else "xgboost")
    if create_stub:
        copyfile(readme_stub, readme)
        pyproject_parsed = tomllib.loads(pyproject)
        pyproject = pyproject.replace(VERSION, str(Version(pyproject_parsed["project"]["version"])))
    elif use_suffix == "cpu":
        copyfile(readme_cpu, readme)
    else:
        copyfile(readme_dft, readme)
    pyproject = pyproject.replace(NCCL, NCCL_WHL.format(require_nccl_dep) if require_nccl_dep != "na" else "")
    pyproject = (
        f"# Generated by `{os.path.basename(__file__)}`, don't edit: "
        f"'--use-suffix={use_suffix} --require-nccl-dep={require_nccl_dep}" + (" --create-stub" if create_stub else "") + "'\n" + pyproject
    )
