...
* ``manylinux2_28``: for recent Linux distros with glibc 2.28 or newer. This variant comes with all features enabled.
...
Starting from **May 31, 2024**, we will stop distributing the ``manylinux2014`` variant and exclusively
distribute the ``manylinux2_28`` variant. We made this decision so that our CI/CD pipeline won't have
