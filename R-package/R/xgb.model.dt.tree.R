...
#'   (called with parameter `with_stats = TRUE`, and called on a model with numeric-only
#'   features). `text` takes precedence over `model`.
#'
#'   Note that, if this dump was produced from a model with categorical features, an
...
xgb.model.dt.tree <- function(model = NULL, text = NULL,
  ...
  if (!is.null(model) && xgb.has_categ_features(model)) {
