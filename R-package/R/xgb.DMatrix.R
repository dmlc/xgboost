...
#' @return An 'xgb.DataIterator' object, containing the same inputs supplied here, which can then
#' be passed to \link{xgb.ExternalDMatrix}.
#' @seealso \link{xgb.ExternalDMatrix}, \link{xgb.ProxyDMatrix}.
#' @export
xgb.DataIterator <- function(env = new.env(), f_next, f_reset) {
...
.single.data.iterator <- function(env) {
  env[["iter"]] <- 0L
  return(xgb.DataIterator(env, .qdm.single.fnext, .qdm.single.freset))
}
...
#' @seealso \link{xgb.DataIterator}, \link{xgb.ExternalDMatrix}.
#' @export
xgb.ProxyDMatrix <- function(
  ...
  if (!inherits(proxy_handle, "xgb.ProxyDMatrix")) {
    stop("'proxy_handle' is invalid. Can only take proxy handles supplied by 'xgb.DataIterator'.")
  }
...
#' @inheritParams xgb.DMatrix
#' @param data_iterator A data iterator structure as returned by \link{xgb.DataIterator},
...
#' }
#' @seealso \link{xgb.DataIterator}, \link{xgb.ProxyDMatrix}, \link{xgb.QuantileDMatrix}
#' @examples
#' library(xgboost)
#' data(mtcars)
#'
...
xgb.ExternalDMatrix <- function(
  ...
  if (!inherits(data_iterator, "xgb.DataIterator")) {
    stop("'data_iterator' must be an 'xgb.DataIterator' object as returned by said function.")
  }
