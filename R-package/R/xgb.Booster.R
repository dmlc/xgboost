...
#' @param predinteraction Whether to return contributions of feature interactions to
#'   individual predictions (see Details).
#' @param reshape Whether to reshape the vector of predictions to matrix form when there
#'   are several prediction outputs per case. No effect if `predleaf`, `predcontrib`,
#'   or `predinteraction` is `TRUE`.
#' @param training Whether the prediction result is used for training. For dart booster,
#'   training predicting will perform dropout.
#' @param iterationrange Sequence of rounds/iterations from the model to use for
#'   prediction, specified by passing a two-dimensional vector with the start and end
#'   numbers in the sequence (same format as R's `seq` - i.e. base-1 indexing, and
#'   inclusive of both ends).
#'
#'   For example, passing `c(1, 20)` will predict using the first twenty iterations,
#'   while passing `c(1, 1)` will predict using only the first one.
#'
#'   If passing `NULL`, will either stop at the best iteration if the model used early
#'   stopping, or use all of the iterations (rounds) otherwise.
#'
#'   If passing "all", will use all of the rounds regardless of whether the model had
#'   early stopping or not.
#' @param strict_shape Default is `FALSE`. When set to `TRUE`, the output type and shape
#'   of predictions are invariant to the model type.
