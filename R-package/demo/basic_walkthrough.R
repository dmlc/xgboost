...
cat("Training xgboost with sparseMatrix")
bst <- xgboost(data = train$data, label = train$label, max.depth = 2, eta = 1, nround = 2,
               nthread = 2, objective = "binary:logistic")
# alternatively, you can put in dense matrix, i.e. basic R-matrix
cat("Training xgboost with Matrix")
...
cat(paste("sum(abs(pred2-pred))=", sum(abs(pred2-pred))))
...
# pred2 should be identical to pred
cat(paste("sum(abs(pred3-pred))=", sum(abs(pred2-pred))))
...
cat('Train xgboost using xgb.train with watchlist')
bst <- xgb.train(data=dtrain, max.depth=2, eta=1, nround=2, watchlist=watchlist,
                 nthread = 2, objective = "binary:logistic")
# we can change evaluation metrics, or use multiple evaluation metrics
cat('Train xgboost using xgb.train with watchlist, watch logloss and error')
bst <- xgb.train(data=dtrain, max.depth=2, eta=1, nround=2, watchlist=watchlist,
...
cat("Most important features (look at column Gain):")
