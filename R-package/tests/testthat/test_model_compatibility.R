  ...
  lapply(list.files(model_dir), function (x) {
    ...
    cpp_warning <- capture.output({
      ...
      if (is_rds) {
        ...
        if (compareVersion(model_xgb_ver, '1.1.1.1') < 0) {
          expected_error_regex <- paste0('.*It is no longer possible to load models from old ',
                                         'versions \\(1\\.0\\.0 or earlier\\) of XGBoost via ',
                                         'readRDS\\(\\).*')
        } else {
          expected_error_regex <- '.*It is not possible to load an old model from RDS.*'
        }
        expect_error(predict(booster, newdata = pred_data), regexp = expected_error_regex)
      } else {
    ...
    if (is_json) {
      # Expect a C++ warning when a model was generated in version < 1.6
      m <- grepl(paste0('.*Found JSON model saved before XGBoost 1\\.6, please save the model ',
                        'using current version again\\.'), cpp_warning, perl = TRUE)
