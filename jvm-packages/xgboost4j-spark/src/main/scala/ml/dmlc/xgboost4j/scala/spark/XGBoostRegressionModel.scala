...
class XGBoostRegressionModel private[spark](override val uid: String, _booster: Booster)
  ...
  private def unwrapFloatArray(array: Array[Float]): Float = {
    array(0)
  }
  ...
  override protected def transformImpl(testSet: Dataset[_]): DataFrame = {
    ...
    val unwrappingUDF = udf(unwrapFloatArray _).apply(col("prediction"))
    testSet.withColumn("prediction", unwrappingUDF)
  }
