...
object PreXGBoost extends PreXGBoostProvider {
  ...
  private val optionProvider: Option[PreXGBoostProvider] = {
    ...
    // TODO find a better way to handle provider
    if (iter.isEmpty) None else Some(iter.head)
  }
  ...
  override def transformSchema(pipeline: PipelineStage, schema: StructType): StructType = {
    ...
    if (optionProvider.isDefined && optionProvider.get.providerEnabled(None)) {
      return optionProvider.get.transformSchema(pipeline, schema)
    }
    ...
    pipeline match {
      ...
      case _ => throw new RuntimeException("Unsupporting " + pipeline)
    }
  }
