...
class XGBoostGeneralSuite extends SharedSparkContext with Utils {
  ...
  test("test eval functions with RDD") {
    // work around unstable prediction for now
    val trainingRDD = buildTrainingRDD(sc).repartition(1)
    val paramMap = List("eta" -> "1", "max_depth" -> "2", "silent" -> "1",
      "objective" -> "binary:logistic").toMap
    val xgBoostModel = XGBoost.trainWithRDD(trainingRDD, paramMap, round = 5, nWorkers = numWorkers)
    xgBoostModel.eval(trainingRDD, "eval1", iter = 5, useExternalCache = true)
    xgBoostModel.eval(trainingRDD, "eval2", evalFunc = new EvalError, useExternalCache = true)
    cleanExternalCache("XGBoostSuite")
  }
