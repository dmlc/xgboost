...
class XGBoostGeneralSuite extends FunSuite with PerTest {
  ...
  test("train with multiple validation datasets (non-ranking)") {
    ...
    assert(model1.summary.validationObjectiveHistory.toSet.map(_._1) === Set("eval1, eval2"))
    ...
    assert(model2.summary.validationObjectiveHistory.toSet.map(_._1) === Set("eval1, eval2"))
