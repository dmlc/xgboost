...
public class XGBoost {
  ...
  /**
   * Load a new Booster model from a file opened as input stream. The assumption is the input stream
   * only contains one XGBoost Model. This can be used to load existing booster models saved by
   * other xgboost bindings.
   *
   * @param in The input stream of the file, will be closed after this function call.
   ...
   *     iteration
   * @param earlyStoppingRound if non-zero, training would be stopped after a specified number of
   *     consecutive increases in any evaluation metric.
   * @param obj customized objective
   * @param eval customized evaluation
   * @return The trained booster.
   */
  ...
  public static BoosterResults trainWithResults(
    ...
    for (int iter = booster.getVersion() / 2; iter < round; iter++) {
      ...
      if (evalMats.length > 0) {
        ...
        if (!decreasing) {
          Rabit.trackerPrint(
              String.format("early stopping after %d decreasing rounds", earlyStoppingRound));
          break;
        }
    ...
    BoosterResults results =
        new BoosterResults(booster, logLines.toArray(new String[logLines.size()]));
    return results;
  }
   ...
   * @throws XGBoostError native error
   */
  private static CVPack[] makeNFold(
      DMatrix data, int nfold, Map<String, Object> params, String[] evalMetrics)
      throws XGBoostError {
