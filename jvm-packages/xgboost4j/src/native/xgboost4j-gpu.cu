...
XGB_DLL int XGQuantileDMatrixCreateFromCallbackImpl(JNIEnv *jenv, jclass, jobject jdata_iter,
  ...
  if (use_ext_mem) {
    ...
    ret = XGQuantileDMatrixCreateFromCallback(&proxy, proxy.GetDMatrixHandle(), ref,
                                                   ExternalMemoryReset, ExternalMemoryNext,
                                                   config, &result);
  } else {
    xgboost::jni::DataIteratorProxy proxy(jdata_iter);
    ret = XGQuantileDMatrixCreateFromCallback(&proxy, proxy.GetDMatrixHandle(), ref, Reset,
                                                   Next, config, &result);
  }
