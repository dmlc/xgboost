...
import ml.dmlc.xgboost4j.scala.spark.rapids.{GpuParams, GpuSampler, GpuTransform, GpuUtils, GpuXGBoost}
...
class XGBoostClassifier (
  ...
  extends ProbabilisticClassifier[Vector, XGBoostClassifier, XGBoostClassificationModel]
    with XGBoostClassifierParams with DefaultParamsWritable with GpuParams {
...
class XGBoostClassificationModel private[ml](
  ...
  extends ProbabilisticClassificationModel[Vector, XGBoostClassificationModel]
    with XGBoostClassifierParams with InferenceParams with GpuParams
