...
function(format_gencode_flags flags out)
  ...
  if (CMAKE_VERSION VERSION_GREATER_EQUAL "3.18")
    ...
    set(modified_flags ${flags})
    list(TRANSFORM modified_flags APPEND "-real")
    list(APPEND modified_flags ${latest_arch})
    set(CMAKE_CUDA_ARCHITECTURES ${modified_flags})
...
macro(xgboost_link_nccl target)
  if (PRUNE_NCCL_ARCHITECTURES)
    set(MODIFIED_NCCL_LIBRARY ${CMAKE_BINARY_DIR}/libnccl_pruned_static.a)
    set(PRUNE_COMMAND "nvprune ${GEN_CODE} ${NCCL_LIBRARY} -o ${MODIFIED_NCCL_LIBRARY}")
    message("Pruning nccl: ${PRUNE_COMMAND}")
    execute_process(COMMAND bash -c "${PRUNE_COMMAND}" RESULT_VARIABLE PRUNE_RESULT)
    if(NOT PRUNE_RESULT EQUAL "0")
        message(FATAL_ERROR "${PRUNE_COMMAND} failed with ${PRUNE_RESULT}.")
    endif()
  else ()
    set(MODIFIED_NCCL_LIBRARY ${NCCL_LIBRARY})
  endif (PRUNE_NCCL_ARCHITECTURES)
  if (BUILD_STATIC_LIB)
    target_include_directories(${target} PUBLIC ${NCCL_INCLUDE_DIR})
    target_compile_definitions(${target} PUBLIC -DXGBOOST_USE_NCCL=1)
    target_link_libraries(${target} PUBLIC ${MODIFIED_NCCL_LIBRARY})
  else ()
    target_include_directories(${target} PRIVATE ${NCCL_INCLUDE_DIR})
    target_compile_definitions(${target} PRIVATE -DXGBOOST_USE_NCCL=1)
    target_link_libraries(${target} PRIVATE ${MODIFIED_NCCL_LIBRARY})
