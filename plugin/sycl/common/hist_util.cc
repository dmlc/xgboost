...
void InitHist(::sycl::queue* qu, GHistRow<GradientSumT, MemoryType::on_device>* hist, size_t size,
              ::sycl::event* event) {
  *event =
      qu->fill(hist->Begin(), xgboost::detail::GradientPairInternal<GradientSumT>(), size, *event);
}
template void InitHist(::sycl::queue* qu, GHistRow<float, MemoryType::on_device>* hist, size_t size,
                       ::sycl::event* event);
template void InitHist(::sycl::queue* qu, GHistRow<double, MemoryType::on_device>* hist,
...
template void CopyHist(::sycl::queue* qu, GHistRow<float, MemoryType::on_device>* dst,
                       const GHistRow<float, MemoryType::on_device>& src, size_t size);
template void CopyHist(::sycl::queue* qu, GHistRow<double, MemoryType::on_device>* dst,
                       const GHistRow<double, MemoryType::on_device>& src, size_t size);
...
::sycl::event ReduceHist(::sycl::queue* qu, GradientPairT* hist_data,
                         GradientPairT* hist_buffer_data, size_t nblocks, size_t nbins,
...
::sycl::event BuildHistKernel(::sycl::queue* qu, const HostDeviceVector<GradientPair>& gpair,
  ...
  auto event_main = qu->submit([&](::sycl::handler& cgh) {
    ...
    cgh.parallel_for<>(
        ::sycl::nd_range<2>(::sycl::range<2>(nblocks, work_group_size),
  ...
  auto event_save = ReduceHist(qu, hist_data, hist_buffer_data, nblocks, nbins, event_main);
...
::sycl::event BuildHistKernelLocal(::sycl::queue* qu, const HostDeviceVector<GradientPair>& gpair,
                                   const RowSetCollection::Elem& row_indices,
                                   const GHistIndexMatrix& gmat,
                                   GHistRow<FPType, MemoryType::on_device>* hist,
                                   GHistRow<FPType, MemoryType::on_device>* hist_buffer,
    ...
    cgh.parallel_for<>(::sycl::nd_range<2>(::sycl::range<2>(nblocks, work_group_size),
                       ...
                       [=](::sycl::nd_item<2> pid) {
                         ...
                         for (size_t fid = feat; fid < n_columns; fid += work_group_size) {
                           ...
                           // Not all elements of hist_fast are actually used: n_bins_feature <=
                           // kMaxNumBins We initililize only the requared elements to prevent the
                           // unused go to cache.
...
::sycl::event BuildHistDispatchKernel(::sycl::queue* qu,
                                      ...
                                      const RowSetCollection::Elem& row_indices,
                                      const GHistIndexMatrix& gmat,
                                      GHistRow<FPType, MemoryType::on_device>* hist, bool isDense,
                                      GHistRow<FPType, MemoryType::on_device>* hist_buffer,
                                      const DeviceProperties& device_prop,
                                      ::sycl::event events_priv, bool force_atomic_use) {
  const size_t size = row_indices.Size();
  const size_t n_columns = isDense ? gmat.nfeatures : gmat.row_stride;
  const size_t nbins = gmat.nbins;
  ...
  auto dispatcher = tree::HistDispatcher<FPType>(device_prop, isDense, size, max_n_blocks, nbins,
  ...
  if (!use_atomic) {
    if (isDense) {
      if (dispatcher.use_local_hist) {
        return BuildHistKernelLocal<FPType, BinIdxType>(qu, gpair, row_indices, gmat, hist,
                                                        hist_buffer, dispatcher, events_priv);
      } else {
        return BuildHistKernel<FPType, BinIdxType, true>(qu, gpair, row_indices, gmat, hist,
                                                         hist_buffer, dispatcher, events_priv);
      }
    } else {
      return BuildHistKernel<FPType, uint32_t, false>(qu, gpair, row_indices, gmat, hist,
                                                      hist_buffer, dispatcher, events_priv);
    }
  } else {
    if (isDense) {
...
::sycl::event BuildHistKernel(::sycl::queue* qu, const HostDeviceVector<GradientPair>& gpair,
                              ...
                              GHistRow<FPType, MemoryType::on_device>* hist,
                              GHistRow<FPType, MemoryType::on_device>* hist_buffer,
                              const DeviceProperties& device_prop, ::sycl::event event_priv,
                              bool force_atomic_use) {
  const bool is_dense = isDense;
  switch (gmat.index.GetBinTypeSize()) {
    case BinTypeSize::kUint8BinsTypeSize:
      return BuildHistDispatchKernel<FPType, uint8_t>(qu, gpair, row_indices, gmat, hist, is_dense,
                                                      hist_buffer, device_prop, event_priv,
                                                      force_atomic_use);
      break;
    case BinTypeSize::kUint16BinsTypeSize:
      return BuildHistDispatchKernel<FPType, uint16_t>(qu, gpair, row_indices, gmat, hist, is_dense,
                                                       hist_buffer, device_prop, event_priv,
                                                       force_atomic_use);
      break;
    case BinTypeSize::kUint32BinsTypeSize:
      return BuildHistDispatchKernel<FPType, uint32_t>(qu, gpair, row_indices, gmat, hist, is_dense,
                                                       hist_buffer, device_prop, event_priv,
                                                       force_atomic_use);
      break;
    default:
...
::sycl::event GHistBuilder<GradientSumT>::BuildHist(
    const HostDeviceVector<GradientPair>& gpair, const RowSetCollection::Elem& row_indices,
    const GHistIndexMatrix& gmat, GHistRowT<MemoryType::on_device>* hist, bool isDense,
    GHistRowT<MemoryType::on_device>* hist_buffer, const DeviceProperties& device_prop,
    ::sycl::event event_priv, bool force_atomic_use) {
  return BuildHistKernel<GradientSumT>(qu_, gpair, row_indices, gmat, isDense, hist, hist_buffer,
                                       device_prop, event_priv, force_atomic_use);
}

template ::sycl::event GHistBuilder<float>::BuildHist(
    const HostDeviceVector<GradientPair>& gpair, const RowSetCollection::Elem& row_indices,
    const GHistIndexMatrix& gmat, GHistRow<float, MemoryType::on_device>* hist, bool isDense,
    GHistRow<float, MemoryType::on_device>* hist_buffer, const DeviceProperties& device_prop,
    ::sycl::event event_priv, bool force_atomic_use);
template ::sycl::event GHistBuilder<double>::BuildHist(
    const HostDeviceVector<GradientPair>& gpair, const RowSetCollection::Elem& row_indices,
    const GHistIndexMatrix& gmat, GHistRow<double, MemoryType::on_device>* hist, bool isDense,
    GHistRow<double, MemoryType::on_device>* hist_buffer, const DeviceProperties& device_prop,
    ::sycl::event event_priv, bool force_atomic_use);
...
template void GHistBuilder<float>::SubtractionTrick(
    GHistRow<float, MemoryType::on_device>* self,
    const GHistRow<float, MemoryType::on_device>& sibling,
    const GHistRow<float, MemoryType::on_device>& parent);
template void GHistBuilder<double>::SubtractionTrick(
    GHistRow<double, MemoryType::on_device>* self,
