...
class HistDispatcher {
 public:
  ...
  HistDispatcher(const DeviceProperties& device_prop, bool isDense, size_t size, size_t max_nblocks,
                 size_t nbins, size_t ncolumns, size_t max_num_bins, size_t min_num_bins) {
    ...
    use_local_hist = (buff_size < device_prop.sram_size_per_eu - KLocalHistSRAM) && isDense &&
                     (max_num_bins <= KMaxNumBins) && (block.size >= th_block_size);
    ...
    float atomic_penalty =
        conflicts_per_bin + kAtomicQuadraticWeight * (conflicts_per_bin * conflicts_per_bin);

    // Block-based builder operates with buffer of type FPType, placed in L2.
    float base_block_penalty =
        kBlockPenaltyNormalization / device_prop.l2_size_per_eu * (sizeof(FPType) / 4);
    ...
    if (use_atomics) {
      work_group_size = std::min(kMaxWorkGroupSizeAtomic, work_group_size);
    } else if (use_local_hist) {
      work_group_size = std::min(kMaxWorkGroupSizeLocal, work_group_size);
    }
  }
};
...
size_t GetRequiredBufferSize(const DeviceProperties& device_prop, size_t max_n_rows, size_t nbins,
  ...
  auto build_params = HistDispatcher<FPType>(device_prop, true, max_n_rows, max_nblocks, nbins,
