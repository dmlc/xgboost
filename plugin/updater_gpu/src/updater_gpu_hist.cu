...
HistHelper DeviceHist::GetBuilder() { return HistHelper(data.data(), n_bins); }

HistHelper::HistHelper(bst_gpair_precise* ptr, int n_bins)
...
__device__ void HistHelper::Add(bst_gpair_precise gpair, int gidx,
                                int nidx) const {
...
__device__ bst_gpair_precise HistHelper::Get(int gidx, int nidx) const {
   ...
   public:
    ...
    void InitData(const std::vector<bst_gpair>& gpair, DMatrix& fmat,  // NOLINT
      ...
      if (!initialised) {
        ...
        dh::safe_nccl(
            ncclCommInitAll(comms.data(), n_devices,
        ...
        h_feature_segments.push_back(n_nodes_level(param.max_depth - 1) *
                                     n_bins);
        ...
        int find_split_n_devices =
            std::pow(2, std::floor(std::log2(n_devices)));
        ...
        for (int d_idx = 0; d_idx < n_devices; d_idx++) {
          ...
          bst_ulong num_elements_segment = device_element_segments[d_idx + 1] -
                                           device_element_segments[d_idx];
          ba.allocate(
              device_idx, param.silent, &(hist_vec[d_idx].data),
              n_nodes(param.max_depth - 1) * n_bins, &nodes[d_idx],
              n_nodes(param.max_depth), &nodes_temp[d_idx],
              max_num_nodes_device, &nodes_child_temp[d_idx],
              max_num_nodes_device, &left_child_smallest[d_idx],
              n_nodes(param.max_depth), &left_child_smallest_temp[d_idx],
              max_num_nodes_device, &feature_flags[d_idx],
          ...
          prediction_cache[d_idx].fill(
              0);  // init device object (assumes comes
      ...
      for (int d_idx = 0; d_idx < n_devices; d_idx++) {
        ...
        device_gpair[d_idx].copy(
            gpair.begin() + device_row_segments[d_idx],
      ...
      if (depth > 0) {
        for (int d_idx = 0; d_idx < n_devices; d_idx++) {
          ...
          dh::launch_n(device_idx, n_sub_bins, [=] __device__(int idx) {
            ...
            bst_gpair_precise parent =
                hist_builder.Get(gidx, parent_nidx(nidx));
...
#define MAX_BLOCK_THREADS 1024
    ...
    void LaunchFindSplit(int depth) {
      ...
      int simuljob =
          1;  // whether to do job on single GPU and broadcast (0) or to
              // do same job on each GPU (1) (could make user parameter,
              // but too fine-grained maybe)
      int findsplit_shardongpus =
          0;  // too expensive generally, disable for now

      if (findsplit_shardongpus) {
        dosimuljob = 0;
        // use power of 2 for split finder because nodes are power of 2
        // (broadcast result to remaining devices)
        int find_split_n_devices =
            std::pow(2, std::floor(std::log2(n_devices)));
        find_split_n_devices =
            std::min(n_nodes_level(depth), find_split_n_devices);
        ...
        // NOTE: No need to scatter before gather as all devices have same copy
        // of nodes, and within find_split_kernel() nodes_temp is given values
        // from nodes
        ...
        for (int d_idx = 0; d_idx < find_split_n_devices; d_idx++) {
          ...
          if (depth !=
              param.max_depth) {  // don't copy over children nodes if no
                                  // more nodes
            dh::safe_nccl(ncclAllGather(
        ...
        if (n_devices > find_split_n_devices && n_devices > 1) {
          ...
          // copies
          // from master to all nodes in case extra devices not involved in
          // split
          for (int d_idx = 0; d_idx < n_devices; d_idx++) {
            int device_idx = dList[d_idx];
            dh::safe_cuda(cudaSetDevice(device_idx));
            ...
            dh::safe_nccl(ncclBcast(
                reinterpret_cast<void*>(nodes[d_idx].data() +
                                        n_nodes(depth - 1)),
            ...
            if (depth !=
              ...
              dh::safe_nccl(ncclBcast(
                  reinterpret_cast<void*>(nodes[d_idx].data() + n_nodes(depth)),
                  n_nodes_level(depth + 1) * sizeof(Node) / sizeof(char),
...
#pragma omp parallel for num_threads(n_devices)
      ...
      for (int d_idx = 0; d_idx < n_devices; d_idx++) {
        ...
        dh::launch_n(device_idx, end - begin, [=] __device__(size_t local_idx) {
          ...
          int gidx = d_gidx[local_idx * static_cast<size_t>(n_columns) +
                            static_cast<size_t>(node.split.findex)];
    ...
    void ColSampleTree() {
      if (param.colsample_bylevel == 1.0 && param.colsample_bytree == 1.0)
        return;
    ...
    bool UpdatePredictionCache(const DMatrix* data,
      ...
      for (int d_idx = 0; d_idx < n_devices; d_idx++) {
        ...
        dh::launch_n(device_idx, prediction_cache[d_idx].size(),
                     [=] __device__(int local_idx) {
                       int pos = d_position[local_idx];
                       d_prediction_cache[local_idx] +=
                           d_nodes[pos].weight * eps;
                     });
