...
class PredictorOneAPI : public Predictor {
 public:
  explicit PredictorOneAPI(GenericParameter const* generic_param) :
    ...
    const DeviceSelector::Specification& device_spec = generic_param->device_selector.Predict();
    ...
    if (is_cpu) {
      ...
      predictor_backend_.reset(Predictor::Create("oneapi_predictor_backend", generic_param));
    }
  }
...
class PredictorOneAPIBackend : public Predictor {
 protected:
  void InitOutPredictions(const MetaInfo& info,
                          HostDeviceVector<bst_float>* out_preds,
                          const gbm::GBTreeModel& model) const {
 ...
 public:
  explicit PredictorOneAPIBackend(GenericParameter const* generic_param) :
...
XGBOOST_REGISTER_PREDICTOR(PredictorOneAPIBackend, "oneapi_predictor_backend")
.describe("Make predictions using DPC++.")
.set_body([](GenericParameter const* generic_param) {
            return new PredictorOneAPIBackend(generic_param);
          });
}  // namespace predictor
}  // namespace xgboost