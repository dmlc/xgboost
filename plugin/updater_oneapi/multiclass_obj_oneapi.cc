...
class SoftmaxMultiClassObjOneAPI : public ObjFunction {
 ...
 public:
  ...
  void Configure(Args const& args) override {
    ...
    if (is_cpu) {
      if (output_prob_) {
        objective_backend_.reset(ObjFunction::Create("multi:softprob", tparam_));
      } else {
        objective_backend_.reset(ObjFunction::Create("multi:softmax", tparam_));
      }
    } else {
      if (output_prob_) {
        objective_backend_.reset(ObjFunction::Create("multi:softprob_oneapi_backend", tparam_));
      } else {
        objective_backend_.reset(ObjFunction::Create("multi:softmax_oneapi_backend", tparam_));
      }
    }
  ...
  void EvalTransform(HostDeviceVector<bst_float>* io_preds) override {
    objective_backend_->EvalTransform(io_preds);
  }
...
class SoftmaxMultiClassObjOneAPIBackend : public ObjFunction {
 private:
  DeviceManagerOneAPI device_manager;

 public:
  explicit SoftmaxMultiClassObjOneAPIBackend(bool output_prob)
  : output_prob_(output_prob) {}
...
XGBOOST_REGISTERATE_DEVICE_SELECTOR_KERNEL("multi:softmax", DeviceType::kOneAPI_Auto, "multi:softmax_oneapi");
XGBOOST_REGISTERATE_DEVICE_SELECTOR_KERNEL("multi:softmax", DeviceType::kOneAPI_CPU,  "multi:softmax_oneapi");
XGBOOST_REGISTERATE_DEVICE_SELECTOR_KERNEL("multi:softmax", DeviceType::kOneAPI_GPU,  "multi:softmax_oneapi");
XGBOOST_REGISTER_OBJECTIVE(SoftmaxMultiClassOneAPIBackend, "multi:softmax_oneapi_backend")
.describe("Softmax for multi-class classification, output class index.")
.set_body([]() { return new SoftmaxMultiClassObjOneAPIBackend(false); });
...
XGBOOST_REGISTERATE_DEVICE_SELECTOR_KERNEL("multi:softprob", DeviceType::kOneAPI_Auto, "multi:softprob_oneapi");
XGBOOST_REGISTERATE_DEVICE_SELECTOR_KERNEL("multi:softprob", DeviceType::kOneAPI_CPU,  "multi:softprob_oneapi");
XGBOOST_REGISTERATE_DEVICE_SELECTOR_KERNEL("multi:softprob", DeviceType::kOneAPI_GPU,  "multi:softprob_oneapi");
XGBOOST_REGISTER_OBJECTIVE(SoftprobMultiClassOneAPIBackend, "multi:softprob_oneapi_backend")
.describe("Softmax for multi-class classification, output probability distribution.")
.set_body([]() { return new SoftmaxMultiClassObjOneAPIBackend(true); });
