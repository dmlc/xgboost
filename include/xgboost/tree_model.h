 ...
 public:
  ...
  struct FVec {
   public:
     ...
     */
    inline void Fill(const data::SparsePage::Inst& inst);
    /*!
     * \brief drop the trace after fill, must be called after fill.
     * \param inst The sparse instance to drop.
     */
    inline void Drop(const data::SparsePage::Inst& inst);
    /*!
...
inline void RegTree::FVec::Fill(const data::SparsePage::Inst& inst) {
...
inline void RegTree::FVec::Drop(const data::SparsePage::Inst& inst) {
  for (bst_uint i = 0; i < inst.length; ++i) {
    if (inst[i].index >= data_.size()) continue;
