 ...
 public:
  ...
  class Node {
   public:
     ...
     */
    XGBOOST_DEVICE void SetLeaf(float value, int right = kInvalidNodeId) {
   ...
   private:
    ...
    bst_node_t parent_{kInvalidNodeId};
    // pointer to left, right
    bst_node_t cleft_{kInvalidNodeId}, cright_{kInvalidNodeId};
   ...
   */
  void ExpandNode(bst_node_t nid, unsigned split_index, float split_value,
                  bool default_left, float base_weight,
                  float left_leaf_weight, float right_leaf_weight,
                  float loss_change, float sum_hess, float left_sum,
                  float right_sum,
  ...
  void ExpandCategorical(bst_node_t nid, bst_feature_t split_index,
                         ...
                         float base_weight, float left_leaf_weight,
                         float right_leaf_weight, float loss_change, float sum_hess,
   ...
   * @param nid node id
   */
  [[nodiscard]] bst_node_t GetDepth(bst_node_t nid) const;
   ...
   * @brief get maximum depth
   */
  [[nodiscard]] bst_node_t MaxDepth() const { return MaxDepth(0); }
  ...
  struct FVec {
     ...
     */
    [[nodiscard]] float GetFvalue(size_t i) const;
    /**
