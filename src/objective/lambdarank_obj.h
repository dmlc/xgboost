 ...
 * labels (relevance degree) provided by the user. Another one is generated by our model,
 * sorted based on prediction scores. `rank_high` refers to the position index of the model
 * rank list that is higher than `rank_low`, while `idx_high` refers to where the
 * `rank_high` sample comes from. Simply put, `rank_high` indexes into the rank list obtained
 * from the model, while `idx_high` indexes into the user-provided sample list.
 */
...
LambdaGrad(linalg::VectorView<float const> labels, common::Span<float const> predts,
           common::Span<size_t const> sorted_idx, std::size_t rank_high,
           std::size_t rank_low,  // higher/lower index on the model rank list
  ...
  return GradientPair{static_cast<float>(lambda_ij), static_cast<float>(hessian_ij)};
}
