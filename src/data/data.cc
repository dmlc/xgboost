...
void MetaInfo::SaveBinary(dmlc::Stream *fo) const {
  XGBoostVersionT major{XGBOOST_VER_MAJOR};
  XGBoostVersionT minor{XGBOOST_VER_MINOR};
  XGBoostVersionT patch{XGBOOST_VER_PATCH};
...
void MetaInfo::LoadBinary(dmlc::Stream *fi) {
  ...
  CHECK(fi->Read(&major, sizeof(major)) == sizeof(major));
  CHECK(fi->Read(&minor, sizeof(major)) == sizeof(minor));
  CHECK(fi->Read(&patch, sizeof(major)) == sizeof(patch));

  // MetaInfo is saved in `SparsePageSource'.  So the version in MetaInfo represents the
  // version of DMatrix.
  CHECK_GE(major, 1) << "Binary DMatrix generated by XGBoost: "
                     << major << "." << minor << "." << patch << " is not supported";
