...
EllpackPageImpl::EllpackPageImpl(AdapterT* adapter, float missing, bool is_dense, int nthread,
  ...
  *this = EllpackPageImpl(adapter->DeviceIdx(), cuts, is_dense, row_stride,
  ...
  data::WriteNullValues(this, adapter->DeviceIdx(), row_counts_span);
}
...
EllpackPageImpl::EllpackPageImpl(AdapterBatch batch, float missing, int device,
  ...
  *this = EllpackPageImpl(device, cuts, is_dense, row_stride, n_rows);
  ...
  data::WriteNullValues(this, device, row_counts_span);
}

template
EllpackPageImpl::EllpackPageImpl(data::CupyAdapterBatch batch, float missing, int device, bool is_dense, int nthread,
                                 bool is_row_major,
                                 common::Span<size_t> row_counts_span,
                                 size_t row_stride, size_t n_rows, common::HistogramCuts cuts);
