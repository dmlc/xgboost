...
class MultiHistogramBuilder {
  ...
  bool ReadByColumn(const GHistIndexMatrix &gidx, bool force_read_by_column) const {
     ...
     * If the histogram is too large to fit in L2 cache, random access becomes a major performance
     * bottleneck.
     *
     * or dense data, using column-wise histogram construction,
     * each iteration of the outer (column-wise) loop accesses only a localized portion of the
     * histogram: idx_bin = gradient_index(row_id, col_id) + offset[col_id]. This improves cache
     * locality, so the column-wise kernel outperforms the row-wise kernel in this case.
     */
