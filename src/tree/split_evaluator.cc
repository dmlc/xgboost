...
bst_float SplitEvaluator::ComputeSplitLoss(bst_uint parentID,
                                           bst_uint featureID,
                                           const GradStats& left,
                                           const GradStats& right) const {
  return ComputeLoss(parentID, left) + ComputeLoss(parentID, right);
}
...
class RidgePenalty : public SplitEvaluator {
 public:
...
class MonotonicConstraint : public RidgePenalty {
 public:
  ...
  bst_float ComputeSplitLoss(bst_uint parentID,
    ...
    bst_float rightWeight = ComputeWeight(parentID, right);

    if(constraint == 0) {
      return loss;
    }
    else if(constraint > 0) {
      return leftWeight <= rightWeight ? loss : infinity;
    }
    else {
      return leftWeight >= rightWeight ? loss : infinity;
    }
