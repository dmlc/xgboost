...
struct TrainParam : public XGBoostParameter<TrainParam> {
  ...
  float learning_rate;
  // depth-wise decay factor for effective learning rate.
  float depth_decay;
  ...
  DMLC_DECLARE_PARAMETER(TrainParam) {
    ...
    DMLC_DECLARE_FIELD(depth_decay)
        .set_range(0.0f, 1.0f)
        .set_default(1.0f)
        .describe(
            "Decay factor for depth-wise learning rate. Effective per-node learning rate "
            "at depth d is learning_rate * depth_decay^d.");
