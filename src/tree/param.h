...
/**
 * @brief Whether both children satisfy min_child_weight using the mean value.
 */
template <typename TrainingParams, typename T>
XGBOOST_DEVICE bool IsValidSplit(TrainingParams const &p, T left_sum_hess, T right_sum_hess,
                                 bst_target_t n_targets) {
  auto min_hess = static_cast<T>(n_targets) * static_cast<T>(p.min_child_weight);
  return left_sum_hess > 0.0 && right_sum_hess > 0.0 && left_sum_hess >= min_hess &&
         right_sum_hess >= min_hess;
}
