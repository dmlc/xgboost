...
DMLC_REGISTRY_FILE_TAG(updater_gpu_hist2);
#endif  // !defined(GTEST_TEST)

// training parameters specific to this algorithm
struct GPUHistMaker2TrainParam
    : public XGBoostParameter<GPUHistMaker2TrainParam> {
  bool single_precision_histogram;
  bool deterministic_histogram;
  bool debug_synchronize;
  // declare parameters
  DMLC_DECLARE_PARAMETER(GPUHistMaker2TrainParam) {
...
#if !defined(GTEST_TEST)
DMLC_REGISTER_PARAMETER(GPUHistMaker2TrainParam);
#endif  // !defined(GTEST_TEST)
...
struct GPUHistMaker2Device {
  ...
  GPUHistMaker2Device(int _device_id,
...
template <typename GradientSumT>
class GPUHistMaker2Specialised {
 public:
  GPUHistMaker2Specialised() = default;
  void Configure(const Args& args, GenericParameter const* generic_param) {
    ...
    monitor_.Init("updater_gpu_hist2");
  }

  ~GPUHistMaker2Specialised() {  // NOLINT
  ...
  void InitDataOnce(DMatrix* dmat) {
    ...
    maker.reset(new GPUHistMaker2Device<GradientSumT>(device_,
                                                     page,
                                                     info_->num_row_,
                                                     param_,
  ...
  std::unique_ptr<GPUHistMaker2Device<GradientSumT>> maker;  // NOLINT

 private:
  bool initialised_ { false };

  GPUHistMaker2TrainParam hist_maker_param_;
...
class GPUHistMaker2 : public TreeUpdater {
 public:
  void Configure(const Args& args) override {
    ...
    if (hist_maker_param_.single_precision_histogram) {
      float_maker_.reset(new GPUHistMaker2Specialised<GradientPair>());
      float_maker_->param_ = param;
      float_maker_->Configure(args, tparam_);
    } else {
      double_maker_.reset(new GPUHistMaker2Specialised<GradientPairPrecise>());
  ...
  char const* Name() const override {
    return "grow_gpu_hist2";
  }

 private:
  GPUHistMaker2TrainParam hist_maker_param_;
  std::unique_ptr<GPUHistMaker2Specialised<GradientPair>> float_maker_;
  std::unique_ptr<GPUHistMaker2Specialised<GradientPairPrecise>> double_maker_;
};

#if !defined(GTEST_TEST)
XGBOOST_REGISTER_TREE_UPDATER(GPUHistMaker2, "grow_gpu_hist2")
    .describe("Grow tree with GPU.")
    .set_body([]() { return new GPUHistMaker2(); });
#endif  // !defined(GTEST_TEST)
