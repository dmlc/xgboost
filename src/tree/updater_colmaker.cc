...
struct NodeEntry {
  ...
  /*! \brief current best solution */
  SplitEntry best;
  friend std::ostream& operator<<(std::ostream& os, NodeEntry const& e) {
    os << "stats: " << e.stats << ", "
       << "root_gain: " << e.root_gain << ", "
       << "weight: " << e.weight << ", "
...
class Builder {
 public:
  ...
  explicit Builder(const TrainParam &param,
                   ...
                   const std::vector<float> &column_densities, common::Monitor* monitor)
     : param_(param), colmaker_train_param_{colmaker_train_param},
       nthread_(omp_get_max_threads()),
       spliteval_(std::move(spliteval)), interaction_constraints_{std::move(
           _interaction_constraints)},
       column_densities_(column_densities), moniter_{monitor} {
  }
...
#pragma omp parallel for schedule(static)
  ...
  void EnumerateSplit(const Entry *begin, const Entry *end, int d_step,
                      bst_uint fid, const std::vector<GradientPair> &gpair,
    ...
    for (const Entry *it = begin; it != end; it += d_step) {
      ...
      if (!IsFresh(nid) || !interaction_constraints_.Query(nid, fid)) {
        continue;
      }
      // start working
...
#pragma omp parallel for schedule(static)
  ...
  std::vector<bst_node_t> position_;
  ...
  void Update(HostDeviceVector<GradientPair> *gpair,
    ...
    for (auto tree : trees) {
      Builder builder(
        ...
        interaction_constraints_, column_densities_, &moniter_);
