 ...
 private:
  ...
  PartitionNodes CreatePartitionNodes(RegTree const* p_tree,
    ...
    for (std::size_t i = 0, n = candidates.size(); i < n; i++) {
      ...
      nodes.left_nidx[i] = sc_tree.LeftChild(e.nid);
      nodes.right_nidx[i] = sc_tree.RightChild(e.nid);

      RegTree::Node split_node = sc_tree.nodes[e.nid];
      nodes.split_data[i] =
 ...
 public:
  ...
  void EvaluateSplits(DMatrix const* p_fmat, const std::vector<GPUExpandEntry>& candidates,
    ...
    for (std::size_t i = 0; i < candidates.size(); i++) {
      ...
      int left_nidx = tree.LeftChild(candidate.nid);
      int right_nidx = tree.RightChild(candidate.nid);
      nidx[i * 2] = left_nidx;
  ...
  void FinalisePosition(DMatrix* p_fmat, RegTree const* p_tree, ObjInfo task,
    ...
    for (auto const& page : p_fmat->GetBatches<EllpackPage>(ctx_, StaticBatch(true))) {
      ...
      for (std::size_t i = 0, n = split_data.size(); i < n; ++i) {
        RegTree::Node split_node = sc_tree.nodes[i];
        auto split_type = sc_tree.SplitType(i);
