...
struct GPUHistMakerDevice {
 ...
 public:
  ...
  GPUHistMakerDevice(Context const* ctx, TrainParam const* _param,
      ...
      : evaluator_{*_param, static_cast<bst_feature_t>(info.num_col_), ctx->Device()},
        ...
        sampler{std::make_unique<GradientBasedSampler>(ctx, info.num_row_, param->subsample,
  ...
  [[nodiscard]] DMatrix* Reset(HostDeviceVector<GradientPair> const* dh_gpair, DMatrix* p_fmat) {
    ...
    this->sampler = std::make_unique<GradientBasedSampler>(ctx_, info.num_row_, param->subsample,
                                                           param->sampling_method);
