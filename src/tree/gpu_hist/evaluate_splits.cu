...
XGBOOST_DEVICE float LossChangeMissing(
    const GradientPairT& scan, const GradientPairT& missing,
    const GradientPairT& parent_sum, const GPUTrainingParam& param,
...
__device__ void EvaluateFeature(
  ...
  uint32_t gidx_end =  inputs.feature_segments[fidx + 1];  // end bin for i^th feature

  // Sum histogram bins for current feature
  GradientSumT const feature_sum =
      ReduceFeature<BLOCK_THREADS, ReduceT, TempStorageT, GradientSumT>(
      inputs.gradient_histogram.subspan(gidx_begin, gidx_end - gidx_begin), temp_storage);
  ...
  for (int scan_begin = gidx_begin; scan_begin < gidx_end;
    ...
    if (thread_active) {
      gain = LossChangeMissing(bin, missing, inputs.parent_sum, inputs.param,
                               inputs.monotonic_constraints[fidx], inputs.value_constraint, missing_left);
    }
    ...
    if (threadIdx.x == block_max.key) {
      ...
      best_split->Update(gain, missing_left ? kLeftDir : kRightDir, fvalue,
                         fidx, GradientPair(left), GradientPair(right), inputs.param);
    }
    __syncthreads();
  }
}
...
__global__ void EvaluateSplitsKernel(
  ...
  int fidx =
      inputs.feature_set[is_left ? blockIdx.x : blockIdx.x - left.feature_set.size()];
                    ...
                    EvaluateSplitInputs<GradientSumT> left, EvaluateSplitInputs<GradientSumT> right)
{
  ...
  std::vector<DeviceSplitCandidate> testing(feature_best_splits.data(),feature_best_splits.data()+feature_best_splits.size());
...
template void EvaluateSplits<GradientPair>(common::Span<DeviceSplitCandidate> out_splits,
  EvaluateSplitInputs<GradientPair> left, EvaluateSplitInputs<GradientPair> right);
template void EvaluateSplits<GradientPairPrecise>(common::Span<DeviceSplitCandidate> out_splits,
  EvaluateSplitInputs<GradientPairPrecise> left, EvaluateSplitInputs<GradientPairPrecise> right);
template void EvaluateSingleSplit<GradientPair>(common::Span<DeviceSplitCandidate> out_split,
    EvaluateSplitInputs<GradientPair> input);
template void EvaluateSingleSplit<GradientPairPrecise>(common::Span<DeviceSplitCandidate> out_split,
