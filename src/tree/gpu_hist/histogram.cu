...
#include <cuda/iterator>         // for counting_iterator, make_counting_iterator
 ...
 public:
  ...
  void BuildHistogram(Context const* ctx, Accessor const& matrix,
    ...
    if (ridxs.size() == 1 && n_total_samples == matrix.n_rows) {
      ...
      using RidxIter = cuda::counting_iterator<cuda_impl::RowIndexT>;
      CHECK_LT(matrix.base_rowid, std::numeric_limits<cuda_impl::RowIndexT>::max());
      auto iter = common::IterSpan{
          cuda::make_counting_iterator(static_cast<cuda_impl::RowIndexT>(matrix.base_rowid)),
          matrix.n_rows};
