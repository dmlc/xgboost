...
struct CachedGpairIter {
  GradientPairInt64 const* XGBOOST_RESTRICT gpairs;
  __device__ void Slice(bst_idx_t n_samples, bst_target_t target_idx) {
    gpairs += (n_samples * target_idx);
  }

  __device__ GradientPairInt64 Load(cuda_impl::RowIndexT ridx) {
    static_assert(sizeof(int4) == sizeof(GradientPairInt64));
    auto g = *reinterpret_cast<int4 const*>(gpairs + ridx);
    return *reinterpret_cast<GradientPairInt64*>(&g);
  }
};

struct MtGpairIter {
  GradientPair const* XGBOOST_RESTRICT gpairs;
  GradientQuantiser const* XGBOOST_RESTRICT roundings;

  __device__ void Slice(bst_idx_t n_samples, bst_target_t target_idx) {
    gpairs += (n_samples * target_idx);
    roundings += target_idx;
  }

  __device__ GradientPairInt64 Load(cuda_impl::RowIndexT ridx) const {
    static_assert(sizeof(int2) == sizeof(GradientPair));
    auto v = *reinterpret_cast<int2 const*>(gpairs + ridx);
    auto g = *reinterpret_cast<GradientPair const*>(&v);
    return roundings->ToFixedPoint(g);
  }
};
...
struct StGpairIter {
  ...
  __device__ GradientPairInt64 Load(cuda_impl::RowIndexT ridx) const {
    ...
    auto v = *reinterpret_cast<int2 const*>(gpairs + ridx);
    auto g = *reinterpret_cast<GradientPair const*>(&v);
    return rounding.ToFixedPoint(g);
  }
};

// Build the histogram for a single target in a single node.
template <typename Policy, typename Accessor, typename RidxIterSpan, typename GpairIter>
__device__ void HistKernelOneNodeTarget(Accessor const& matrix, FeatureGroup const& group,
                                        RidxIterSpan d_ridx_iter, GpairIter gpair,
 ...
 */
template <typename Policy, typename Accessor, typename GpairIter>
__global__ __launch_bounds__(HistBound::kBlockThreads, HistBound::kMinBlocks) void StHistKernel(
    ...
    common::Span<cuda_impl::RowIndexT const> d_ridx_iter, GpairIter d_gpair,
  ...
  HistKernelOneNodeTarget<Policy>(matrix, group, d_ridx_iter, d_gpair, smem_hist, node_hist.data(),
                                  offset, kStride);
}
...
__global__ __launch_bounds__(HistBound::kBlockThreads, HistBound::kMinBlocks) void MtHistKernel(
    ...
    GpairIter d_gpair, bst_idx_t n_samples, bst_target_t n_targets) {
  using Idx = RowPartitioner::RowIndexT;
  ...
  d_gpair.Slice(n_samples, target_idx);
...
struct HistKernel {
  ...
  template <bool kDense, bool kCompressed, typename Accessor, typename GpairIter>
  void DispatchHistShmem(Context const* ctx, Accessor const& matrix,
                         FeatureGroupsAccessor const& feature_groups, GpairIter gpair,
    ...
    if (use_shared) {
      DispatchCudaSm(ctx->Ordinal(), [&](auto arch) {
        ...
        auto kernel = StHistKernel<Policy, Accessor, GpairIter>;
  ...
  void DispatchHistShmem(Context const* ctx, Accessor const& matrix,
                         ...
                         linalg::MatrixView<GradientPair const> gpair, GpairIter giter,
    ...
    auto launch = [&](auto policy, auto kernel) {
      ...
      dim3 conf(n_blocks, feature_groups.NumGroups());
      kernel<<<conf, Policy::kBlockThreads, shmem_bytes, ctx->CUDACtx()->Stream()>>>(
          matrix, feature_groups, ridx_iters, dh::ToSpan(blk_ptr), hists.data(), giter, n_samples,
          n_targets);
      dh::safe_cuda(cudaPeekAtLastError());
    };
    ...
    if (use_shared) {
      DispatchCudaSm(ctx->Ordinal(), [&](auto arch) {
        using Arch = common::GetValueT<decltype(arch)>;
        using Policy = HistPolicy<Arch, kItemsPerThread, kDense, kCompressed, true>;
        auto kernel = MtHistKernel<Policy, Accessor, RidxIterSpan, GpairIter>;
        this->SetCfg(Policy{}, shmem_bytes, kernel);
        launch(Policy{}, kernel);
      });
    } else {
...
class DeviceHistogramDispatchAccessor {
  ...
  std::unique_ptr<linalg::Matrix<GradientPairInt64>> cache_grad_;

 public:
  void Reset(Context const* ctx, bool force_global_memory, bool cache_grad,
             common::Span<GradientQuantiser const> roundings,
             linalg::MatrixView<GradientPair const> gpairs) {
    this->kernel_ = std::make_unique<HistKernel>(ctx, force_global_memory);

    this->cache_grad_.reset();
    if (cache_grad) {
      this->cache_grad_ = std::make_unique<linalg::Matrix<GradientPairInt64>>();
      CalcQuantizedGpairs(ctx, gpairs, roundings, cache_grad_.get());
    }
  }
  ...
  void BuildHistogram(Context const* ctx, Accessor const& matrix,
                      ...
                      common::Span<cuda_impl::RowIndexT const> ridx,
                      common::Span<GradientPairInt64> hist, GradientQuantiser const& rounding) {
    if (this->cache_grad_) {
      auto cache_grad = this->cache_grad_->View(ctx->Device());
      CHECK(cache_grad.FContiguous());
      auto giter = CachedGpairIter{cache_grad.Values().data()};
      this->kernel_->Dispatch(ctx, matrix, feature_groups, giter, ridx, hist);
    } else {
      auto giter = StGpairIter{gpair.data(), rounding};
      this->kernel_->Dispatch(ctx, matrix, feature_groups, giter, ridx, hist);
    }
  }
  ...
  void BuildHistogram(Context const* ctx, Accessor const& matrix,
    ...
    if (ridxs.size() == 1 && n_total_samples == matrix.n_rows) {
      ...
      if (this->cache_grad_) {
        ...
        CHECK(cache_grad.FContiguous());
        auto giter = CachedGpairIter{cache_grad.Values().data()};
        this->kernel_->Dispatch(ctx, matrix, feature_groups, gpair, giter, ridx_iters.data().get(),
                                hists, h_sizes_csum);
      } else {
        auto giter = MtGpairIter{gpair.Values().data(), roundings.data()};
        this->kernel_->Dispatch(ctx, matrix, feature_groups, gpair, giter, ridx_iters.data().get(),
                                hists, h_sizes_csum);
      }
    } else {
      ...
      if (this->cache_grad_) {
        ...
        CHECK(cache_grad.FContiguous());
        auto giter = CachedGpairIter{cache_grad.Values().data()};
        this->kernel_->Dispatch(ctx, matrix, feature_groups, gpair, giter, ridxs.data(), hists,
                                h_sizes_csum);
      } else {
        auto giter = MtGpairIter{gpair.Values().data(), roundings.data()};
        this->kernel_->Dispatch(ctx, matrix, feature_groups, gpair, giter, ridxs.data(), hists,
...
void DeviceHistogramBuilder::Reset(Context const* ctx, std::size_t max_cached_hist_nodes,
                                   ...
                                   bool cache_grad, common::Span<GradientQuantiser const> roundings,
                                   linalg::MatrixView<GradientPair const> gpairs) {
  this->monitor_.Start(__func__);
  this->p_impl_->Reset(ctx, force_global_memory, cache_grad, roundings, gpairs);
