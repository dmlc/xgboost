 ...
 protected:
  ...
  void InitModelUserParam(LearnerTrainParam const& tparam, CacheT const& cache) {
    ...
    if (this->NeedFit()) {
      ...
      auto t = this->mparam_.OutputLength();
      if (t == 1) {
        this->mparam_.base_score.Resize(t, ObjFunction::DefaultBaseScore());
      } else {
        this->mparam_.base_score.Resize(this->mparam_.OutputLength(), 1.0 / static_cast<double>(t));
      }
