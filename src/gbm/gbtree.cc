...
struct DartTrainParam : public dmlc::Parameter<DartTrainParam> {
  ...
  DMLC_DECLARE_PARAMETER(DartTrainParam) {
    DMLC_DECLARE_FIELD(silent).set_default(0)
        .describe("Not print information during trainig.");
    DMLC_DECLARE_FIELD(samp_drop).set_default(0)
        .describe("Different types of sampling algorithm.");
    DMLC_DECLARE_FIELD(norm_drop).set_default(0)
...
class Dart : public GBTree {
 public:
  ...
  void Load(dmlc::Stream* fi) override {
    ...
    if (mparam.num_trees != 0) {
      CHECK_EQ(fi->Read(dmlc::BeginPtr(weight_drop), sizeof(float) * mparam.num_trees),
               sizeof(float) * mparam.num_trees)
          << "Dart: invalid model file";;
    }
  }
  ...
  void Save(dmlc::Stream* fo) const override {
    ...
    if (weight_drop.size() != 0) {
      fo->Write(dmlc::BeginPtr(weight_drop), sizeof(float) * tree_info.size());
    }
  }
 ...
 protected:
  ...
  inline size_t NormalizeTrees(size_t size_new_trees) {
    ...
    float factor = 1.0 * num_drop / (num_drop + 1.0);
    if (num_drop == 0) {
      for (size_t i = 0; i < size_new_trees; ++i) {
        weight_drop.push_back(1.0);
      }
    } else {
      if (dparam.norm_drop == 2) {
        // norm_drop 2
        float fl = 1.0 * num_drop / (num_drop + dparam.learning_rate);
        for (size_t i = 0; i < idx_drop.size(); ++i) {
          weight_drop[i] *= fl;
        }
        for (size_t i = 0; i < size_new_trees; ++i) {
          weight_drop.push_back(fl);
        }
      } else if (dparam.norm_drop == 1) {
        // norm_drop 1
        for (size_t i = 0; i < idx_drop.size(); ++i) {
        ...
        for (size_t i = 0; i < size_new_trees; ++i) {
          weight_drop.push_back(1.0 / (num_drop + 1.0));
        }
      }
    }
    // reset
