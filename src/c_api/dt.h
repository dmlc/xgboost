 ...
 * \file dt.h
 * \brief Datatable handling for C API.
 */

#ifndef XGBOOST_C_API_DT_H_
#define XGBOOST_C_API_DT_H_
...
template <>
inline int16_t GETNA() {
  return NA_I2;
}
template <>
inline int32_t GETNA() {
  return NA_I4;
}
template <>
inline int64_t GETNA() {
  return NA_I8;
}
...
class Datacol {
 public:
  ...
  int datacoltype;
  int whichj;
  Datacol(void **data, const wchar_t **feature_stypes, int j)
...
bool DtIsMissingAndGetValue(Datacol *d, int i, float *value) {
  // order of likelihood
