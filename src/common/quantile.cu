...
size_t SketchContainer::Unique() {
   ...
   * prunning procedure. We preserve the first type and remove the second type.
   * Preserving the second type is difficult because we are running on multiple sliding
   * windows, so the accumulated weights need to be passed down to later batches. */
  dh::safe_cuda(cudaSetDevice(device_));
  timer_.Start(__func__);
  this->columns_ptr_.SetDevice(device_);
  Span<OffsetT> d_columns_ptr_in = this->columns_ptr_.DeviceSpan();
  CHECK_EQ(d_columns_ptr_in.size(), num_columns_ + 1);
  Span<SketchEntry> entries = dh::ToSpan(this->Current());
  dh::XGBCachingDeviceAllocator<char> alloc;

  dh::caching_device_vector<OffsetT> columns_ptr_out(d_columns_ptr_in.size());
  auto d_columns_ptr_out = dh::ToSpan(columns_ptr_out);
  ...
  auto beg_it = thrust::make_reverse_iterator(tran_it + entries.size());
  // Reverse scan to accumulate weights into first duplicated element on left.
  thrust::inclusive_scan_by_key(
      thrust::cuda::par(alloc), beg_it, beg_it + entries.size(),
      this->Current().rbegin(), this->Current().rbegin(),
      thrust::equal_to<size_t>{},
  ...
  Span<OffsetT const> src = dh::ToSpan(columns_ptr_out);
  CopyTo(d_columns_ptr_in, src);
