...
void GHistIndexBlockMatrix::Build(const GHistIndexMatrix& gmat,
  ...
  std::vector<size_t> index_size (nblock+1, 0);
  index_size[0] = 0;
  ...
  for (size_t rid = 0; rid < nrow; ++rid) {
    ...
    for (uint32_t block_id = 0; block_id < nblock; ++block_id) {
      ...
      size_t const ind = begin + row_ptr_count[block_id + 1] + 1;
      row_ptr_[ind] = index_size[block_id + 1];
  ...
  for (size_t rid = 0; rid < nrow; ++rid) {
    ...
    for (size_t j = ibegin; j < iend; ++j) {
      const uint32_t bin_id = gmat.index[j];
      const uint32_t block_id = bin2block[bin_id];
      size_t block_begin = indices_ptr[block_id];
      index_[block_begin + indices_count[block_id]] = bin_id;
