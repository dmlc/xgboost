...
inline to_chars_result to_chars(char *first, char *last, float value) {  // NOLINT
  if (XGBOOST_EXPECT(!(static_cast<size_t>(last - first) >= NumericLimits<float>::kToCharsSize),
                     false)) {
...
inline from_chars_result from_chars(const char *buffer, const char *end,  // NOLINT
                                    float &value) {                       // NOLINT
  from_chars_result res = detail::FromCharFloatImpl(buffer, std::distance(buffer, end), &value);
  return res;
}
}  // namespace xgboost
