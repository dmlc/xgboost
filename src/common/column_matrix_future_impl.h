...
#ifndef XGBOOST_COMMON_COLUMN_MATRIX_FUTURE_IMPL_H_
#define XGBOOST_COMMON_COLUMN_MATRIX_FUTURE_IMPL_H_
...
class ColumnFImpl {
 public:
  ...
  ColumnFImpl(ColumnType type, BinTypeSize bin_type_size,
...
class SparseColumn: public ColumnFImpl {
 public:
  SparseColumn(BinTypeSize bin_type_size, common::Span<const ByteType> index,
      ...
      : ColumnFImpl(ColumnType::kSparseColumn, bin_type_size, index, index_base),
        row_ind_(row_ind) {}

  const size_t* GetRowData() const { return row_ind_.data(); }

  template <typename BinIdxType, typename CastType = ColumnFImpl::BinCmpType>
...
class DenseColumn: public ColumnFImpl {
 public:
  DenseColumn(BinTypeSize bin_type_size, common::Span<const ByteType> index,
      ...
      : ColumnFImpl(ColumnType::kDenseColumn, bin_type_size, index, index_base),
...
class ColumnViewFImpl final {
 public:
  ColumnViewFImpl() = delete;
  explicit ColumnViewFImpl(const SparseColumn * sparse_clmn_ptr) :
    sparse_clmn_ptr_(sparse_clmn_ptr),
    dense_clmn_ptr_(nullptr) { }
  explicit ColumnViewFImpl(const DenseColumn * dense_clmn_ptr) :
...
/*! \brief a collection of columns, with support for construction from
    GHistIndexMatrix. */
class ColumnMatrixFImpl {
 public:
  using ByteType = uint8_t;
  using ColumnListType = std::vector<std::shared_ptr<const ColumnFImpl>>;
  using ColumnViewListType = std::vector<std::shared_ptr<const ColumnViewFImpl>>;
 ...
 private:
  ...
  void AddColumnToList(size_t fid, Args&& ... args) {
        ...
        column_view_list_[fid] = std::make_shared<const ColumnViewFImpl>(clmn.get());
  }
...
#endif  // XGBOOST_COMMON_COLUMN_MATRIX_FUTURE_IMPL_H_
