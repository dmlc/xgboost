...
Version::TripletT Version::Load(Json const& in) {
  ...
  try {
    ...
    std::tie(major, minor, patch) =
        std::make_tuple(get<Integer const>(j_version.at(0)), get<Integer const>(j_version.at(1)),
...
Version::TripletT Version::Load(dmlc::Stream* fi) {
  ...
  std::string msg{
      "Incorrect version format found in binary file.  "
...
void Version::Save(Json* out) {
  ...
  (*out)["version"] =
      std::vector<Json>{Json(Integer{major}), Json(Integer{minor}), Json(Integer{patch})};
}
...
bool Version::Same(TripletT const& triplet) { return triplet == Self(); }
